...
compatibility: 需要 Node.js/npx、可读写的本地项目目录，以及能访问飞书的 lark-cli；输入必须包含 profile，首次认证需要用户在浏览器完成授权。
---
...
2. 找到后解析 `identity_key` 与 `items`。若 token 重复、路径冲突或机器区与人工表格不一致，停止写入并标记待确认，不自动修复历史。
