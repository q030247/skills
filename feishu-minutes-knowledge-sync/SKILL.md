...
运行环境需要 Python 3、Node.js/npx、可读写的本地项目目录，以及能访问飞书的 `lark-cli`；首次认证需要用户在浏览器完成授权。
...
- 新同步的原始逐字稿必须以`transcript_review_status: pending_review`进入人工校订闸门；同步技能不得替用户勾选放行或改写原始文字记录。后续每日整理只在来源包双边勾选后同步为可提取状态。
...
5. 两篇文档都在 YAML 后、正文最前方写入`- [ ] 整理完毕 <!-- feishu_processing_complete -->`。同步不得自动勾选，也不得在后续运行中覆盖人工状态。两篇都勾选时表示来源包已经人工检查并放行；下一次每日整理同步逐字稿状态并进入提取。单边勾选时列为待确认。它不等同于候选待办批准或归档批准。
6. 智能纪要正文链接 `[[原始逐字稿文件名]]`；逐字稿正文链接 `[[智能纪要文件名]]`。使用不含 `.md` 的 Obsidian 双向链接。
7. 先以临时文件准备两篇笔记，验证 YAML、必填字段、文件名和双链；再写入最终位置。若任一文件失败，删除本次尚未正式登记的临时文件，保留失败信息，不留下单边成功假象。
8. 两篇都成功后才追加索引记录，`sync_status: synced`、`pair_state: complete`。不要把逐字稿临时 `.txt` 当作知识库最终产物。
...
- 两篇新笔记正文最前方均有未勾选的稳定“整理完毕”标记，且同步不会自动勾选或覆盖人工状态；双边人工勾选由每日整理解释为来源包放行。
