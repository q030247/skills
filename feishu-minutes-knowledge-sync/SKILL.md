...
5. 两篇文档都在 YAML 后、正文最前方写入`- [ ] 整理完毕 <!-- feishu_processing_complete -->`。该复选框只表示单篇文档是否已由人工整理；同步不得自动勾选，也不得在后续运行中覆盖人工状态。两篇都勾选时来源包才算整体整理完毕，单边勾选时列为待确认。它不等同于逐字稿校订放行、候选待办批准或归档批准。
...
- 两篇新笔记正文最前方均有未勾选的稳定“整理完毕”标记，且同步不会自动勾选或覆盖人工状态。
